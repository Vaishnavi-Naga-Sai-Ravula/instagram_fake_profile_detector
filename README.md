...
The system is implemented as a complete pipeline that includes:
- Data preprocessing and feature engineering (`main.py`)
- Class balancing using **SMOTE** to handle imbalanced datasets
- Model training and evaluation with multiple classifiers
- Backend API development (`flask_app.py`) using Flask
- Interactive frontend (`app.py`) built with Streamlit
...
👉 The Streamlit dashboard supports dark mode toggle, glassmorphism styling, animations, subtle background movement, caching for performance, overlays, gamified performance metrics, SMOTE class balance visualization, and an explainability tab** alongside interactive charts (heatmaps, pie charts, ROC curves) for predictions and performance visualization.

---

## Key Features
- Multiple machine learning models for comparative analysis
- **Soft Voting Ensemble** for probability-averaged predictions
- **Stacking Ensemble** for meta-learning based predictions
- SMOTE oversampling for balanced training data
- Flask REST API for serving predictions, metrics, and explanations
- Streamlit dashboard with:
  ...
  - Class balance visualization before and after SMOTE
  - Explainability tab with feature values and averaged feature importance
  - Comparative explanation of XGBoost vs ensembles
...
- `soft_voting.pkl` (Soft Voting Ensemble)
...
| XGBoost | Boosting |
| AdaBoost | Boosting |
| Soft Voting Ensemble | Hybrid Ensemble |
...
- `stacking.pkl` – Stacking ensemble model  
- `soft_voting.pkl` – Voting ensemble model  
...
Dataset → Preprocessing → Feature Engineering → SMOTE Balancing → Model Training
→ Soft Voting + Stacking Ensembles → Model Storage → Flask API → Streamlit Interface
...
│
# Trained machine learning models + metrics.json + class_balance.png
├── models/
# Dataset files
├── data/
# Static assets(background image)
├── static/
# Documentation(architecture_diagram,research results)
├── docs/
│
# Training pipeline(SMOTE+generates models+metrices)
├── main.py
...
| /predict/all   | POST   | Returns predictions from all models + explanation |
...
The system provides:
- Classification result (fake or real) with animated overlays
- Probability scores per model
- Soft Voting and Stacking ensemble decision
- Model performance metrics including accuracy, precision, recall, F1 score, and ROC-AUC
- Streamlit dashboard with heatmaps, pie charts, bar charts, and ROC curves
- Class balance visualization before and after SMOTE
- Explainability tab with:
    - Feature values used in prediction
    - Averaged feature importance across models
    - Comparative explanation of ensemble vs XGBoost
    - Dynamic input-based reasoning
---
